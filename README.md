...
## 개발 내용
- 처리 속도와 복잡한 쿼리 요구로 인해 JPA 대신 MyBatis와 저장 프로시저를 사용해서 데이터 처리
- 보안 이슈가 중요하지 않아 개발 속도와 단순한 구조를 위해 DTO를 따로 만들지 않고 Map으로 파라미터 관리
- 중복 로직 제거 및 성능 최적화를 위해 프로시저 호출 방식으로 게시판, 근태관리 등 핵심 기능 구현
- 유지보수성 향상을 위해 기능 단위로 코드 분리해서 가독성 및 유지보수성 개선
- 연차·주간업무 기능에서 공휴일이 필요해 공휴일 API 설계 구현
